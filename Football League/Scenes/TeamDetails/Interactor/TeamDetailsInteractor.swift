...
class TeamDetailsInteractor {
    
    func GetTeamDetailsApi( teamID:Int, completionHandler: @escaping (TeamDetailsModel?, String?) -> ()){
        
       
        let URL = baseUrl + "\(teamDetailsURL)\(teamID)"
        ...
        AFWrapper.requestGETURL(URL, headers: headers, params: nil,success: { ( response) in
            ...
            do {
                let result = try JSONDecoder().decode(TeamDetailsModel.self, from: data)
